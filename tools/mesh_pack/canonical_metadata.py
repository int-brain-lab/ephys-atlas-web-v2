...
UNAVAILABLE_SOURCE_IDS = frozenset({545})
...
def _ancestor_source(
    identifier: int, active: set[int], by_id: dict[int, dict[str, Any]]
) -> int | None:
    current = identifier
    visited: set[int] = set()
...
def canonical_source_assignment(
    catalog: dict[str, Any],
    active_document: dict[str, Any],
) -> tuple[list[int], list[tuple[int, str]], np.ndarray, dict[str, Any]]:
    """Map bilateral LUT row indices onto the reviewed deepest-active source identities."""
    ...
    source_ids = sorted(renderable - UNAVAILABLE_SOURCE_IDS)
    allen_rows = catalog["mappings"]["allen"]
    by_id = {
        int(row["atlas_id"]): row for row in allen_rows if int(row["atlas_id"]) > 0
    }
    target_by_row = np.zeros(
        max(int(row["idx"]) for row in allen_rows) + 1, dtype=np.uint16
    )
    signed_sources = [
        (identifier, hemisphere)
    ...
    return source_ids, signed_sources, target_by_row, scope


def build_metadata(
    lut_path: Path, catalog_path: Path, active_path: Path, output: Path
) -> dict[str, Any]:
    catalog = json.loads(catalog_path.read_bytes())
    active_document = json.loads(active_path.read_bytes())
    _source_ids, signed_sources, target_by_row, _scope = canonical_source_assignment(
    ...
    ml_coordinates = (
        ORIGINS_UM[0] + np.arange(LABEL_SHAPE[1], dtype=np.float64) * SPACING_UM[0]
    )
    dv_coordinates = (
        ORIGINS_UM[2] + np.arange(LABEL_SHAPE[2], dtype=np.float64) * SPACING_UM[2]
    )
    for ap_start in range(0, LABEL_SHAPE[0], 4):
        ...
        ap_coordinates = (
            ORIGINS_UM[1]
            + np.arange(ap_start, ap_stop, dtype=np.float64) * SPACING_UM[1]
        )
        sums[:, 0] += np.bincount(
            flat,
            weights=np.broadcast_to(
                ml_coordinates[None, :, None], targets.shape
            ).reshape(-1),
            minlength=size,
        )
        sums[:, 1] += np.bincount(
            flat,
            weights=np.broadcast_to(
                ap_coordinates[:, None, None], targets.shape
            ).reshape(-1),
            minlength=size,
        )
        sums[:, 2] += np.bincount(
            flat,
            weights=np.broadcast_to(
                dv_coordinates[None, None, :], targets.shape
            ).reshape(-1),
            minlength=size,
        )
        if ap_stop % 80 == 0 or ap_stop == LABEL_SHAPE[0]:
            print(
                f"canonical centroids: {ap_stop}/{LABEL_SHAPE[0]} AP planes", flush=True
            )
    del label

    if np.any(counts[1:] == 0):
        missing = [
            signed_sources[index]
            for index, value in enumerate(counts[1:])
            if value == 0
        ]
        raise ValueError(
            f"renderable signed Allen regions have no canonical voxels: {missing}"
        )
    ...
    for index, (source_id, hemisphere) in enumerate(signed_sources):
        mappings = {
            name: resolve_mapping(source_id, name, catalog)
        ...
        group_count, group_sum = group_totals.get(
            group_key, (0, np.zeros(3, dtype=np.float64))
        )
    ...
    explode_groups = [
        {
            "group_id": group,
            "hemisphere": hemisphere,
            "centroid_um": [round(float(value), 6) for value in total / count],
            "voxel_count": count,
        }
    ...
    document = {
        ...
        "excluded_source_allen_ids": sorted(UNAVAILABLE_SOURCE_IDS),
...
def main() -> None:
    ...
    build_metadata(
        arguments.lut, arguments.catalog, arguments.active_inventory, arguments.output
    )
