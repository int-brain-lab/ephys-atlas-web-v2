...
def _load_parent_slices(
    ...
    for artifact in sorted(pack_set["packs"], key=lambda value: value["pack_index"]):
        ...
        expected_indices = list(range(expected_first, expected_first + len(slices)))
        if [item["slice_index"] for item in slices] != expected_indices:
        ...
        for item in slices:
            ...
            if not math.isclose(
                float(item["world_coordinate_um"]),
                expected,
                abs_tol=1e-6,
                rel_tol=0,
            ):
                raise ValueError(
                    f"parent affine/world mismatch at {projection_name}:{item['slice_index']}"
                )
            lattice_position = (
                float(item["world_coordinate_um"]) - lattice_anchor
            ) / spacing_um
            if math.isclose(
                lattice_position,
                round(lattice_position),
                abs_tol=1e-8,
            ):
                result.append(item)
...
def _pack_id(
    ...
    digest = _sha(_canonical(identity))[:12]
    return f"{parent_pack_id}-display-{spacing_um}um-d{pack_depth}-{digest}"
...
def _write_projection(
    ...
    selected, native_count = _load_parent_slices(
        parent_root,
        parent,
        name,
        spacing_um,
    ...
    for start in range(0, len(selected), pack_depth):
        ...
        pack = SvgPack(
            ...
            tuple(
                SvgFragment(
                    int(slice_value["slice_index"]),
                    float(slice_value["world_coordinate_um"]),
                    _fragment(slice_value),
                )
                for slice_value in chunk
            ),
        )
        raw = encode(pack)
        if decode(raw) != pack:
        ...
        rel = _safe_relative(
            (Path("packs") / str(pack_depth) / name / f"{pack_index}.isvg.gz").as_posix()
        )
        target = stage / rel
        ...
        artifacts.append(
            {
                "pack_id": artifact_pack_id,
                "pack_index": pack_index,
                "first_display_index": start,
                "first_slice_index": chunk[0]["slice_index"],
    ...
    return {
        "fixed_world_axis": parent_projection["fixed_world_axis"],
        "plane_axes": parent_projection["plane_axes"],
        "slice_count": native_count,
        "display_slice_count": len(selected),
        "display_slice_indices": [int(item["slice_index"]) for item in selected],
        "slice_shape": parent_projection["slice_shape"],
        "view_box": parent_projection["view_box"],
        "plane_index_to_world_um": parent_projection["plane_index_to_world_um"],
        "world_to_plane_index": parent_projection["world_to_plane_index"],
        "lattice_anchor_slice_index": anchor_index,
        "lattice_origin_um": anchor,
        "lattice_spacing_um": spacing_um,
        "pack_sets": {
            str(pack_depth): {
                "pack_depth": pack_depth,
                "path_template": f"packs/{pack_depth}/{name}/{{pack}}.isvg.gz",
                "packs": artifacts,
            }
        },
        "validation": {
            "native_source_slices": native_count,
            "display_emitted_slices": len(selected),
        },
    }
...
def _manifest(
    ...
    return {
        ...
        "parent": {
            ...
            "synchronization_sentinels": parent.get("synchronization_sentinels"),
        },
        "provenance": {
            "generator": {
            ...
            "derivation": "byte-preserving SVG fragment extraction from validated parent anatomy-pack-v2",
        },
        "sampling": {
            "native_resolution_um": 10,
            "spacing_um": spacing_um,
            "lattice": "native plane nearest fixed-axis world zero plus integer spacing",
            "pack_depth": pack_depth,
        },
        "coordinate_system": parent["coordinate_system"],
        "projections": projections,
        "validation": {
            "native_source_slices": sum(
                item["slice_count"] for item in projections.values()
            ),
            "display_emitted_slices": sum(
                item["display_slice_count"] for item in projections.values()
            ),
        },
    }
...
def build_sampled(
    ...
    *,
    ...
    if spacing_um <= 0 or spacing_um % 10:
        raise ValueError(
            "spacing_um must be a positive multiple of the native 10 um grid"
        )
    ...
    if resolved_output == parent_root or parent_root in resolved_output.parents:
