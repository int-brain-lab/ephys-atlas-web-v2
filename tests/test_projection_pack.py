...
def _inputs(root: Path, *, valid_parent: bool = True) -> tuple[Path, Path, dict[str, Path]]:
...
def test_projection_pack_is_deterministic_and_complete(tmp_path: Path) -> None:
    ...
    assert [projection["id"] for projection in manifests[0]["projections"]] == [
        "coronal", "sagittal", "horizontal", "top", "swanson"
    ]
    assert sorted(path.relative_to(outputs[0]) for path in outputs[0].rglob("*") if path.is_file()) == sorted(
    ...
    for first in (path for path in outputs[0].rglob("*") if path.is_file()):
        assert first.read_bytes() == (outputs[1] / first.relative_to(outputs[0])).read_bytes()
    ...
    for projection in manifests[0]["projections"][:3]:
        ...
        parent_projection = json.loads((registered / "manifest.json").read_text())["projections"][
            projection["id"]
        ]
        assert projection["slice_shape"] == list(reversed(parent_projection["slice_shape"]))

    static = {projection["id"]: projection for projection in manifests[0]["projections"][3:]}
    for name, count in (("top", 114), ("swanson", 808)):
        resource = static[name]["fragment"]["resource"]
        fragment = gzip.decompress((outputs[0] / resource["path"]).read_bytes()).decode()
    ...
    assert all(not source.get("path", "").startswith("/") for source in manifests[0]["provenance"]["sources"])


def test_complete_pack_validator_rejects_tampering_and_undeclared_files(tmp_path: Path) -> None:
    registered, catalog, sources = _inputs(tmp_path)
    ...
    top = next(projection for projection in manifest["projections"] if projection["id"] == "top")
...
def test_static_normalization_rejects_markup_and_unknown_legacy_ids(tmp_path: Path) -> None:
...
def test_production_static_ingestion_requires_license_and_pinned_bytes(tmp_path: Path) -> None:
    registered, catalog, sources = _inputs(tmp_path)
