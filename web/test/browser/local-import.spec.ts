...
test('validated ZIP preview is read-only until confirmation and persists locally', async ({ page }) => {
  ...
  await expect(dataTrigger).toContainText('Stored only in this browser');
  await dataTrigger.click();
  const dataDialog = page.getByRole('listbox', { name: 'Data', exact: true });
  await expect(dataDialog.getByRole('group', { name: 'My data' })).toBeVisible();
