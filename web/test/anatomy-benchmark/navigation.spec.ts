...
test('retained projection viewport meets registered navigation timing gates', async ({ page, browserName }) => {
  ...
  const measurements = await page.evaluate(async ({ cases, manifestUrl, trials }) => {
    const [{ ProjectionPackSource }, { RetainedProjectionViewportFactory }] = await Promise.all([
      import('/src/rendering/projection-pack-source.ts'),
      import('/src/rendering/retained-projection-viewport.ts'),
    ]);
    const results = [];
    ...
    for (const benchmarkCase of cases) {
      ...
      for (let trial = 0; trial < trials; trial += 1) {
        ...
        const fetchImpl: typeof fetch = (input, init) => {
          ...
          if (url.pathname.endsWith('.isvg.gz')) {
            url.searchParams.set('projection-benchmark', `${benchmarkCase.label}-${trial}`);
          }
          return fetch(url, { ...init, cache: 'no-store' });
        };
        const source = new ProjectionPackSource({ manifestUrl, fetchImpl });
        ...
        const viewport = factory.create(target, benchmarkCase.axis);
        const retainedRoot = target.firstElementChild;
        ...
        const capture = async (sliceIndex: number) => {
          ...
          await viewport.render(model(sliceIndex));
          ...
          return {
            ...
            retainedDom: retainedRoot === target.firstElementChild,
          };
        };
  ...
  const report = {
    benchmark: 'projection-pack-v1-retained-viewport',
  ...
  for (const result of measurements.results) {
    ...
    expect(result.samples.every((sample) => (
      sample.coldPack.retainedDom && sample.samePack.retainedDom && sample.retainedRevisit.retainedDom
    ))).toBe(true);
  }
