import type { SliceAxis, SliceGuide, SliceIndices, ViewBox, WorldCoordinateUm } from './spatial.js';
import { PROJECTION_PLANE_AXES, SLICE_WORLD_AXIS } from './spatial.js';
...
export function regionalIndexToLegacyIndex(axis: SliceAxis, regionalIndex: number): number {
  return coordinateUmToIndex(regionalIndexToCoordinateUm(axis, regionalIndex), REGIONAL_10UM_CALIBRATION[axis]);
}
...
export function legacyAnatomy25UmIndicesToWorld(indices: SliceIndices): WorldCoordinateUm {
  return {
    ml: indexToCoordinateUm(indices.sagittal, ANATOMY_25UM_CALIBRATION.sagittal),
    ap: indexToCoordinateUm(indices.coronal, ANATOMY_25UM_CALIBRATION.coronal),
    dv: indexToCoordinateUm(indices.horizontal, ANATOMY_25UM_CALIBRATION.horizontal),
  };
}

function projectLegacyWorldGuide(sourceAxis: SliceAxis, targetAxis: SliceAxis, world: WorldCoordinateUm): SliceGuide {
...
export function projectLegacyGuide(sourceAxis: SliceAxis, targetAxis: SliceAxis, sourceIndex: number): SliceGuide {
  ...
  const worldAxis = SLICE_WORLD_AXIS[sourceAxis];
  const world: WorldCoordinateUm = { ml: 0, ap: 0, dv: 0, [worldAxis]: indexToCoordinateUm(sourceIndex, source) };
