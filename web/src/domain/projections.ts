...
/** The enabled 2-D projection registry. Static maps join this registry at their UI cutover. */
...
export const PROJECTION_BY_ID: Readonly<Record<OrthogonalProjectionId, OrthogonalProjectionDefinition>> =
  Object.fromEntries(PROJECTION_REGISTRY.map((projection) => [projection.id, projection])) as Record<
    OrthogonalProjectionId,
...
export const WORKSPACE_VIEW_REGISTRY = [
  ...PROJECTION_REGISTRY.map((projection) => ({
