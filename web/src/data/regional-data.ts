...
export function parseRegionMetadata(value: unknown): readonly RegionMetadata[] {
  ...
  const regions = value.map((raw, position) => {
    ...
    else if (typeof item.parent_id === 'number' && Number.isInteger(item.parent_id)) region.parentId = String(item.parent_id);
    else if (typeof item.parent_id === 'string' && item.parent_id) region.parentId = item.parent_id;
    if (typeof item.depth === 'number' && Number.isInteger(item.depth) && item.depth >= 0) region.depth = item.depth;
    return region;
  });
...
function optionalStatistic(source: Record<string, unknown>, sourceKey: string, targetKey: keyof GlobalStatistics, target: GlobalStatistics): void {
...
export function parseRegionalStatisticsResource(value: unknown): RegionalStatisticsResource {
  ...
  if (!Array.isArray(regional.fields)) throw new Error('statistics.regional_summary.fields must be an array');
  const fields = regional.fields.map((field, index) => nonEmptyString(field, `statistics.regional_summary.fields[${index}]`));
  ...
  if (root.global !== undefined) {
    ...
    optionalStatistic(source, 'count', 'count', global);
    optionalStatistic(source, 'missing_count', 'missingCount', global);
    optionalStatistic(source, 'min', 'min', global);
    optionalStatistic(source, 'max', 'max', global);
    optionalStatistic(source, 'mean', 'mean', global);
    optionalStatistic(source, 'std', 'std', global);
    optionalStatistic(source, 'median', 'median', global);
    optionalStatistic(source, 'q05', 'q05', global);
    optionalStatistic(source, 'q25', 'q25', global);
    optionalStatistic(source, 'q75', 'q75', global);
    optionalStatistic(source, 'q95', 'q95', global);
  ...
  if (root.histogram !== undefined) {
    ...
    if (edges.length !== globalCounts.length + 1) throw new Error('histogram edges must be one longer than global counts');
    parsed.histogram = {
      edges,
      globalCounts,
...
export function materializeRegionalHistogram(
  ...
  histogram.regionalCounts = Array.from({ length: regionCount }, (_, row) =>
    flatCounts.slice(row * binCount, (row + 1) * binCount),
  );
  return histogram;
}
