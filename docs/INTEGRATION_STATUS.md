...
D078 successor preserves all source float16 values, classifies only
nonzero-label voxels as valid, adds exact Allen/Beryl/Cosmos companions, passes
schema validation and the real-release Chromium suite, and has deterministic
visual and regional reviews. It is not staged or published; see the
[candidate review](data/AGEA_PROCESSED_REVIEW.md) and
[regional review](data/VOLUME_REGIONAL_DISTRIBUTION_REVIEW.md).
