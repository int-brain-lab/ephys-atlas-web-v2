...
| Optional 3-D context | A sibling retained Three.js viewport shares regional presentation/selection and owns camera, explode, GPU lifecycle, and failure isolation. Volume features remain anatomy-only in 3-D. | D042 real pack is losslessly repackaged and ready locally; Chromium plus owner Safari/Firefox review passed. | D066 [native-component candidate](tasks/2026-09-02-native-3d-mesh-components/README.md) has validated topology auditing and deterministic movement proposals with synthetic tests. D068 fixes side-specific presentation and independent movement; Q18 retains exact assignments and boundary behavior. Candidate pack/runtime changes are unimplemented. D042 cut/cap remains the default. Optional public deployment and experimental-label decision; not launch-blocking. |
