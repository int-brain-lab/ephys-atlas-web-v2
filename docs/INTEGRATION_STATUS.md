...
| Optional 3-D context | A sibling retained Three.js viewport shares regional presentation/selection and owns camera, explode, GPU lifecycle, and failure isolation. Volume features remain anatomy-only in 3-D. | D042 real pack is losslessly repackaged and ready locally; Chromium plus owner Safari/Firefox review passed. | The D066/D068 synthetic vertical slice is implemented: independent presentation/component identities, explicit component displacement, original-world-ML side presentation, and CPU picking that follows shader explode movement. Its exact-midline rule is explicitly provisional and test-only. Q18 still blocks a real native candidate's boundary tolerance and reviewed movements. D042 cut/cap remains the default. Optional public deployment and experimental-label decision; not launch-blocking. |
