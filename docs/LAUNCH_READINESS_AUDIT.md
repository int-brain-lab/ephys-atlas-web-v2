...
| [LS01-08](LAUNCH_SPEC.md#ls01-08) | Public navigation separates project, dataset, immutable release, feature, and view; project editions preserve coordinated release mappings and disclose overrides. | The schema/compiler and catalog-first resolved-navigation slices implement immutable edition identity, exact URL-v4 context, and custom baselines with synthetic tests; the top bar remains flat. | blocked Q9 | Implement the desktop/tablet and narrow/accessibility UI slices, then bind the exact paper edition/default only after Q9 is authorized. |
