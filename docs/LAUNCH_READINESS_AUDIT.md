...
| [LS01-08](LAUNCH_SPEC.md#ls01-08) | Public navigation separates project, dataset, immutable release, feature, and view; project editions preserve coordinated release mappings and disclose overrides. | The schema/compiler, catalog-first resolver, four-field context bar, narrow staged chooser, grouped keyboard behavior, and explicit recovery flows implement immutable edition identity, exact URL-v4 context, custom baselines, release metadata, and history transitions with synthetic tests. | blocked Q9 | Bind the exact paper edition/default only after Q9 is authorized. |
