...
| [LS01-08](LAUNCH_SPEC.md#ls01-08) | Public navigation separates project, dataset, immutable release, feature, and view; project editions preserve coordinated release mappings and disclose overrides. | D056/D061 and `frontend/DATASET_NAVIGATION.md` define the accepted immutable-edition, catalog-authority, resolved-navigation, and responsive product contract; the current catalog and top bar remain flat. | blocked Q9 | Implement and test the four generic slices with synthetic mappings now; bind the exact paper edition/default only after Q9 is authorized. |
