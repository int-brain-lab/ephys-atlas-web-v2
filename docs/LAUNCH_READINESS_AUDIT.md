...
release set or satisfy the full launch browser/performance matrix. D075 closes Q5 with the real CloudFront benchmark; ordinary production volume
publication is the next operation. Q2/Q9 remain paper-only choices.
...
| [LS03-11](LAUNCH_SPEC.md#ls03-11) | Ordinary production single-slice navigation avoids the entire volume. | Candidate request-bounded evidence; production origin is deployed, but ordinary production ephys volume transport is not selected. | independent gap | Complete the production-origin benchmark and freeze transport. |
...
| [LS12-03](LAUNCH_SPEC.md#ls12-03) | Production navigation meets documented request/bytes/decode/memory budget selected from real data. | D075 real CloudFront matrix passes the declared actual-slider, startup, request and byte budgets; production release publication remains pending. | independent gap | Publish and verify the production volume release. |
...
| [LS13-02](LAUNCH_SPEC.md#ls13-02) | CORS and relevant Range behavior verified from production origin. | Same-origin live browsing and sampled HTTP Range 206 pass; this is not a complete documented CORS/header applicability matrix. | independent gap | Record the full origin/header matrix, including why cross-origin CORS is unnecessary for the selected same-origin topology. |
...
| [LS13-04](LAUNCH_SPEC.md#ls13-04) | Secrets stay outside the repository and scientific releases use the canonical Linux preflight. | D062; all four initial releases passed exact-commit clean-Linux preflight with external operator credentials. A dated final secret-scan record is not included in this evidence. | independent gap | Record a secret/config scan and repeat preflight for the final Linux release set. |
...
| [DLR-02](LAUNCH_SPEC.md#dlr-02) | Every launch dataset has an immutable provenance-valid release or waiver. | Four initial scientific datasets have immutable published releases with provenance/preflight evidence. Ordinary production ephys volumes are absent; paper freeze remains separate. | independent gap | Select and publish the production volume release after Q5; resolve Q2/Q9 for the later paper set. |
...
| [DLR-05](LAUNCH_SPEC.md#dlr-05) | No unresolved launch blocker in open questions. | Q8 is closed by D074 live deployment. D075 closes Q5; Q2/Q9 govern only the later paper freeze. | independent gap | Complete publication/QA operations and resolve Q2/Q9 before claiming paper readiness. |
| [DLR-06](LAUNCH_SPEC.md#dlr-06) | Integration status describes shipped state, not plans. | Integration status and frozen initial deployment evidence describe the shipped site, four datasets and remaining volume/QA/paper limitations. | satisfied | Update shipped status again after Q5 and final broad QA. |
