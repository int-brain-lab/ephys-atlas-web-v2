...
| [LS01-08](LAUNCH_SPEC.md#ls01-08) | Public navigation separates project, dataset, immutable release, feature, and view; project editions preserve coordinated release mappings and disclose overrides. | The schema/compiler, catalog-first resolved-navigation, and desktop/tablet four-field context-bar slices implement immutable edition identity, exact URL-v4 context, custom baselines, release metadata, and history transitions with synthetic tests. | blocked Q9 | Implement the narrow/accessibility UI slice, then bind the exact paper edition/default only after Q9 is authorized. |
