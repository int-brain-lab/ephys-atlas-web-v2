...
rollback evidence. Approved-pack website integration is complete; deployment is separate.
...
The active v5 bundle selects `artifacts/mesh-native-d070-v1`, pack
`ibl-native-d070-b5f5abc7d0bb3575`, built on clean Linux commit `3d52114`
with builder `native-selected-v1`. Two builds matched every byte.

- Manifest: 682,717 bytes, SHA-256
  `131d3e8cbac9bdadcab8e9fe1b12d3ad2b9b85632f840c4b2c56db07f52d4973`.
- Geometry: 14,055,506 served bytes, 24,433,484 decoded bytes, SHA-256
...
Build a **new** approved graph from the exact reviewed pack on clean Linux `main`:
...
uv run --project builder --extra test --locked python -m tools.mesh_pack.build_native_selected \
  ...
  --output artifacts/mesh-native-d070-new \
...
The current `native-selected-v2` compiler includes the build commit, environment, and selection
in pack identity, so future provenance changes cannot reuse immutable asset URLs.
To reproduce the exact v5-bundle graph, use its pinned `3d52114` compiler in a
clean Linux checkout; a build from a later commit is a new artifact requiring
a new bundle descriptor, not an in-place replacement.

Rollback uses `data/development-bundle-v5-d042-rollback.json` with the same
datasets and the preserved current-contract D042 pack. See the
[local bundle runbook](../data/LOCAL_DEVELOPMENT_BUNDLE.md#native-anatomy-and-rollback).
...
## Historical D042 compiled-full evidence
