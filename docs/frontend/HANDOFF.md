...
- each scientific context field is its own accessible custom picker: dataset releases are grouped by dataset, the feature catalogue is searchable by label/unit/ID/semantics, and representation plus parcellation share one compact picker; volume mode labels Allen/Beryl/Cosmos as anatomy parcellations because they change the overlay and inspection context without changing voxel values;
